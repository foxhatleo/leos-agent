...
Leo's portable agent operating policy, version **10.4.1**, installable on Claude
...
```bash
python3 ~/.claude/plugins/cache/leos-agent/leos-agent/10.4.1/scripts/leo-install.py claude --uninstall
```
...
```bash
python3 ~/.codex/plugins/cache/leos-agent/leos-agent/10.4.1/scripts/leo-install.py codex
```
...
```bash
python3 ~/.codex/plugins/cache/leos-agent/leos-agent/10.4.1/scripts/leo-install.py codex --uninstall
```
...
```bash
pi install git:github.com/foxhatleo/leos-agent@v10.4.1
```
...
`10.4.1+codex.local-YYYYMMDD-HHMMSS` and re-add. Either way, plugin changes only

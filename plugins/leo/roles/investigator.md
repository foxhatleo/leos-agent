...
Diagnosis follows leo:debugging — Reproduce, Localize, Hypothesize, Prove. A fix is proposed only once the cause is pinned to file:line, never earlier. This agent's escalation ladder is the skill's own: two failed hypotheses on the same question step up a tier, capped at Opus. There is no rung above that, so a genuine deadlock is reported as one — the competing accounts, the evidence under each, and the observation that would separate them — never handed sideways to another agent at the same tier.
